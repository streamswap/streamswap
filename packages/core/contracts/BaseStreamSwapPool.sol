...
import "../balancer-v2-monorepo/pkg/pool-utils/contracts/BaseMinimalSwapInfoPool.sol";
 ...
 */
abstract contract BaseWeightedPool is IMinimalSwapInfoPool, BasePool, WeightedMath {
    using FixedPoint for uint256;
    using WeightedPoolUserDataHelpers for bytes;

    uint256 private _lastInvariant;

    StreamSwapLibrary.Context private streamSwapContext;
    ...
    function onSwap(
        ...
        if (request.kind == IVault.SwapKind.GIVEN_IN) {
            ...
            streamSwapContext.updateFlowRates(streamSwapContext.tokens[indexIn], streamSwapContext.tokens[indexOut], StreamSwapLibrary.Record(
                ...
                streamSwapContext.records[].balance + swapRequest.amount
            ));
            streamSwapContext.updateFlowRates(streamSwapContext.tokens[indexIn], streamSwapContext.tokens[indexOut], StreamSwapLibrary.Record(
                0,0,
                streamSwapContext.records[].denorm,
                streamSwapContext.records[].balance - amountOut
            ));
            ...
            streamSwapContext.updateFlowRates(streamSwapContext.tokens[indexIn], streamSwapContext.tokens[indexOut], StreamSwapLibrary.Record(
                0,0,
                streamSwapContext.records[].denorm,
                streamSwapContext.records[].balance + amountIn
            ));
            streamSwapContext.updateFlowRates(streamSwapContext.tokens[indexIn], streamSwapContext.tokens[indexOut], StreamSwapLibrary.Record(
                0,0,
                streamSwapContext.records[].denorm,
