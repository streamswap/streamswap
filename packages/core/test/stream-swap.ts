...
describe('StreamSwapPool', function() {
  ...
  it.skip('should swap single stream', async () => {

    console.log(encodeStreamSwapData([{
    ...
    console.log(await sf!.getGovernance());
    console.log(await superTokenA!.balanceOf(myAddress));
    console.log(myAddress);

    console.log(cfa!.interface.encodeFunctionData('createFlow', [
      superTokenA!.address,
      streamSwap!.address,
  ...
  it('should swap single stream into 2 outputs', async () => {
    // start by creating all new stream
    //await sf!.callAgreement('', , ethers.utils.defaultAbiCoder.encode());

    // then edit existing stream
  });
  ...
  it('should join and exit', async () => {
    ...
    await streamSwap!.joinPool(wei(2).toBN(), [wei(20).toBN(), wei(20).toBN()]);
    ...
    await streamSwap!.exitPool(wei(2).toBN(), [wei(5).toBN(), wei(5).toBN()]);
